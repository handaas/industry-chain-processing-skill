...
6. **Render and report**
   - Before rendering, require a complete non-empty graph: at least one L2, one L3, and one L5 overall, and every displayed L2/L3 branch must contain children. Reconstruct from `value_chain` or the generated fallback ontology when project graph data is empty; if the graph is still incomplete, fail explicitly and do not create a report file.
