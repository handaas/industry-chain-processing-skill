...
REMOTE_SEARCH_TOOL = "enterprise_get_keyword_search"
KEYWORD_SKIP = {
    "营业",
    "个体户",
    "制造业",
...
def call_remote_enterprise_search_preview(
    ...
    *,
    ...
    """Use existing HandaaS MCP interface wrappers as the data-access layer."""
        ...
        try:
            ...
            for row in normalized:
                ...
                if key and key not in merged:
                    merged[key] = row
            calls.append({"tool": REMOTE_SEARCH_TOOL, "matchKeyword": keyword, "rows": len(normalized)})
    ...
    return {
        ...
        "note": "MCP 只调用 HandaaS 已有接口封装；产业链条件拆解和去重由 Skill 本地完成。",
        "keywords": keywords,
    ...
    if not args.local and has_remote_mcp_config(args.config):
        if args.dry_run:
            print_json({
                ...
                "mode": "mcp",
                "search_tools": [REMOTE_SEARCH_TOOL],
                "keywords": extract_keywords_from_condition(condition, [args.node, args.chain], limit=8),
                "note": "去掉 --dry-run 后将调用 enterprise_get_keyword_search；不会调用自定义工作流工具。",
            })
            return
